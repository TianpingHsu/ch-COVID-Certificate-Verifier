                             ...
                             )
{
    ...
    if (parameters.cose_algorithm_id == -37)  // PS256
    {
        if (phdr && sig) {
            phdr->len = protected_parameters.len;
            phdr->ptr = malloc(protected_parameters.len * sizeof(int8_t));
            memcpy((uint8_t*)phdr->ptr, (uint8_t*)protected_parameters.ptr, protected_parameters.len);

            sig->len = signature.len;
            sig->ptr = malloc(sizeof(int8_t) * signature.len);
            memcpy((uint8_t*)sig->ptr, (uint8_t*)signature.ptr, signature.len);
            goto Done;
        }
    }
